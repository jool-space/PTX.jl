...
#
# pyptx keeps the exp values in registers across passes 2 and 3; this
# Julia port follows rms_norm.jl's convention of reloading from DRAM
# (cached after pass 1) and recomputing exp(x-m) in pass 3 — three loads
# total instead of one. Three-line trade for not needing @generated to
# unroll across the row_sum reduction. Bandwidth-bound either way.
#
...
function _softmax_v4_kernel!(
    ...
    while j < v4_iters
        ...
        row_max = ptx"max.f32"(row_max, x4[1])
        row_max = ptx"max.f32"(row_max, x4[2])
        row_max = ptx"max.f32"(row_max, x4[3])
        row_max = ptx"max.f32"(row_max, x4[4])
        j += 1
    end
    ...
    # --- pass 2: re-load X, compute exp(x - row_max), accumulate row_sum
    row_sum = 0f0
    j = 0
    while j < v4_iters
        idx = elem_base + j * (block * 4)
        off = idx * 4
        x4 = ptx"ld.global.v4.f32"(px + off)
        e1 = ptx"ex2.approx.f32"(ptx"fma.rn.f32"(x4[1], log2e, neg_max_l2))
        e2 = ptx"ex2.approx.f32"(ptx"fma.rn.f32"(x4[2], log2e, neg_max_l2))
        e3 = ptx"ex2.approx.f32"(ptx"fma.rn.f32"(x4[3], log2e, neg_max_l2))
        e4 = ptx"ex2.approx.f32"(ptx"fma.rn.f32"(x4[4], log2e, neg_max_l2))
        row_sum = ptx"add.f32"(row_sum, e1)
        row_sum = ptx"add.f32"(row_sum, e2)
        row_sum = ptx"add.f32"(row_sum, e3)
        row_sum = ptx"add.f32"(row_sum, e4)
        j += 1
    end
    ...
    # --- pass 3: re-load X, recompute exp(x - row_max), multiply, store --
    j = 0
    while j < v4_iters
        idx = elem_base + j * (block * 4)
        off = idx * 4
        ...
        e3 = ptx"ex2.approx.f32"(ptx"fma.rn.f32"(x4[3], log2e, neg_max_l2))
        e4 = ptx"ex2.approx.f32"(ptx"fma.rn.f32"(x4[4], log2e, neg_max_l2))
        y1 = ptx"mul.f32"(e1, inv_sum)
        y2 = ptx"mul.f32"(e2, inv_sum)
        y3 = ptx"mul.f32"(e3, inv_sum)
        y4 = ptx"mul.f32"(e4, inv_sum)
