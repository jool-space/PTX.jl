...
#
# Matches pyptx's headline config: BN=256, BK_TILE=64, N_STAGES=3.
# m64n256k16 wgmma → 128 f32 frags per lane; 3-stage SMEM ring requires
# the non-power-of-two phase cursor that `Pipeline{N}` now supports.
#
# Configuration:
...
#   Dynamic SMEM ~209 KB per CTA (opt-in via FUNC_ATTRIBUTE_MAX_DYNAMIC_SHARED_SIZE_BYTES)
#
...
function _ghh_gemm_kernel!(
    ...
    if wg_id == UInt32(0)
        ...
        while true
            ...
            # m64n256k16 frag layout per lane: 32 col-groups of 4 frags.
            ...
            # Unrolled epilogue stores. 32 groups (BN/8) × 2 packed stores
            # (row_a, row_b). Generated by Base.Cartesian.@nexprs for
            # literal-index tuple access.
            Base.Cartesian.@nexprs 32 g -> begin
...
@testset "gemm_highperf_hopper compiles at sm_90a" begin
    ...
    # 32 col-groups × 2 packs = 64 fused `cvt.rn.bf16x2.f32` per lane.
...
if v"9.0" <= DEV_CAP < v"12.0"
    ...
    @testset "gemm_highperf_hopper (M=512, N=512, K=256, multi-tile + multi-K)" begin
        # 2 m-clusters × 2 n-tiles = 4 tiles. With 2 clusters launched,
        # each cluster does 2 tiles. K=256 → num_blocks_k=4 per tile.
