...
# BN=256 with this kernel's 1-producer + 2-consumer warpgroup layout
# overflows H100's 64K-reg-per-CTA budget at ptxas allocation time —
# consumer's 128-frag accumulator pushes static reg use to ~180/thread
# × 384 threads = ~69K. setmaxnreg.inc.240 is a runtime redistribution,
# not a compile-time cap, so it doesn't fix this. Restructuring to a
# single consumer warpgroup (BM_CTA=64) is a half-day rewrite; not
# pursuing here. The non-pow2 N_STAGES=3 cursor in `Pipeline{N}` IS
# validated end-to-end at BN=128.
#
# Configuration:
