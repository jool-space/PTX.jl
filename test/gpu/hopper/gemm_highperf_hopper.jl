...
#
# Adopts pyptx's headline N_STAGES=3 and BN=256. The original port used
# BN=128 because the @nexprs epilogue's bf16x2_pack + st.shared.b32
# working set kept the accumulator regs and 32 packed regs co-live, and
# NUM_REGS climbed to 180 at BN=256 (180 × 384 = 69,120 > 65,536-reg-per-
# CTA budget → ERROR_LAUNCH_OUT_OF_RESOURCES). Switching the epilogue to
# `stmatrix.sync.aligned.m8n8.x4.trans.shared::cta.b16` drains the
# accumulator progressively (each stmatrix call consumes 4 packed regs
# and frees them) and drops NUM_REGS to 164 at BN=256 → 164 × 384 =
# 62,976 < 65,536. The SMEM C layout becomes (N-row, M-col) row-major
# (matching pyptx) and the TMA C descriptor is built accordingly.
#
# Configuration:
#   BM_CTA = 128, BN = 256, BK_TILE = 64, WGMMA_K = 16, N_STAGES = 3
#   CLUSTERS = 2 (cluster_m=2, cluster_n=1)
#   384 threads per CTA = 1 producer + 2 consumer warpgroups
#   m64n256k16 wgmma — 128 f32 frags per lane
#   WGMMA_KITERS = 4 wgmma sub-calls per K-tile load
#   Dynamic SMEM ~213 KB per CTA (opt-in via FUNC_ATTRIBUTE_MAX_DYNAMIC_SHARED_SIZE_BYTES)
#
...
# `pyptx.ptx.stmatrix_x4_trans_f32_bf16` (pyptx/pyptx/ptx.py:2450).
#
# Consumes 8 f32 frags per stmatrix call, packing them into 4 bf16x2 b32 regs
# and emitting a single warp-cooperative store of 4 m8n8 tiles. With trans,
# the 4 tiles tile a 16-N-row × 16-M-col region of SMEM. With N total wgmma
# fragments per lane, emits `N ÷ 8` stmatrix calls.
#
# SMEM destination layout: (N-row, M-col) row-major with stride `ROW_STRIDE`
...
@inline @generated function _ghh_stmatrix_epilogue!(
    ...
    N % 8 == 0 || error("_ghh_stmatrix_epilogue!: frag length must be a multiple of 8, got $N")
    n_groups = N ÷ 8
    group_stride_bytes = ROW_STRIDE * 16 * 2
    body = Expr[]
    for g in 0:(n_groups - 1)
        base = g * 8
        offset_bytes = g * group_stride_bytes
        push!(body, quote
            let
                pack0 = bf16x2_pack(frag[$(base + 1)], frag[$(base + 2)])
                pack1 = bf16x2_pack(frag[$(base + 3)], frag[$(base + 4)])
                pack2 = bf16x2_pack(frag[$(base + 5)], frag[$(base + 6)])
                pack3 = bf16x2_pack(frag[$(base + 7)], frag[$(base + 8)])
                ptx"stmatrix.sync.aligned.m8n8.x4.trans.shared::cta.b16"(
                    stm_base + $offset_bytes, (pack0, pack1, pack2, pack3))
            end
        end)
    end
    quote
        Base.@inline
        lane32 = lane & UInt32(31)
        warp   = lane >> UInt32(5)
        tid_off = warp * UInt32(16)
        tid_off += (lane32 & UInt32(7)) * UInt32($ROW_STRIDE)
        tid_off += (lane32 >> UInt32(4)) * UInt32($ROW_STRIDE) * UInt32(8)
        tid_off += lane32 & UInt32(8)
        stm_base = smem_base + Int(tid_off) * 2
        $(body...)
        nothing
    end
end
        ...
        while true
            ...
            @inbounds for kt in Int32(0):(num_blocks_k - Int32(1))
                ...
                @inbounds for kk in 0:(GHH_WGMMA_KITERS - 1)
                    ...
                    d = ptx"wgmma.mma_async.sync.aligned.m64n256k16.f32.bf16.bf16"(
            ...
            # SMEM C layout is (N-row, M-col) row-major per consumer; the
            # stmatrix.x4.trans helper writes the wgmma f32 frag tuple in
            # that orientation. Each consumer's slab is BN × B_WG_M bf16,
            # stacked back-to-back at byte offset consumer_id * BN*B_WG_M*2.
            c_consumer_ptr = c_base_ptr + Int(consumer_id) * GHH_BN * Int(GHH_B_WG_M) * 2
            _ghh_stmatrix_epilogue!(c_consumer_ptr, d, lane128, Val(GHH_B_WG_M))
            ...
            # Lane 0 of each consumer issues TMA store of its BN × B_WG_M slice.
            # gmem C is (N-row, M-col) row-major → coord0 = M (inner), coord1 = N.
            if lane128 == UInt32(0)
                ptx"fence.proxy.async.shared::cta"()
                ...
                ptx"cp.async.bulk.tensor.2d.global.shared::cta.tile.bulk_group"(
                    tma_C, store_m, store_n, c_consumer_ptr)
...
@testset "gemm_highperf_hopper compiles at sm_90a" begin
    ...
    @test occursin("wgmma.mma_async.sync.aligned.m64n256k16.f32.bf16.bf16", ptx)
    ...
    @test occursin("stmatrix.sync.aligned.m8n8.x4.trans.shared::cta.b16", ptx)
    # 8 stmatrix groups (BN/16 wgmma cols per group) × 4 fused bf16x2 packs.
...
if test_runtime_supported(@__FILE__)
    function _run_ghh_case(rng, M_total, N_total, K_test, num_clusters)
        ...
        tmap_C = tensor_map_tile_2d(:bf16, pointer(C_d),
            N_total, M_total, GHH_BN, GHH_B_WG_M; swizzle = :NONE)
        ...
        kern = @cuda launch=false _ghh_gemm_kernel!(
        ...
        # gmem C is (N-row, M-col) row-major → M innermost in linear memory.
        # Julia (M, N) col-major reshape matches.
    ...
    @testset "gemm_highperf_hopper (M=256, N=256, K=64, single tile, single cluster)" begin
        # Degenerate: 1 cluster handles 1 tile with 1 K-iter. Smoke test
        # of the persistent loop's exit path on the sentinel.
        C_got, C_ref = _run_ghh_case(MersenneTwister(0x901), 256, 256, 64, 1)
