...
#
# Adopts pyptx's headline N_STAGES=3, but keeps BN=128 (pyptx uses 256).
# Confirmed empirically on GH200 (sm_90a, H100 die) that BN=256 with
# this kernel's 1-producer + 2-consumer warpgroup layout overflows the
# 65,536-reg-per-CTA budget at ptxas allocation time: NUM_REGS climbs
# from 98 at BN=128 to 180 at BN=256, and 180 × 384 = 69,120 > 65,536
# → ERROR_LAUNCH_OUT_OF_RESOURCES. (SMEM opt-in works fine; the 213,552
# byte attribute reads back correctly. SMEM is not the bottleneck.)
# Path to BN=256 is a kernel restructure: single consumer warpgroup at
# BM_CTA=64, dropping to 256 threads/CTA → 180 × 256 = 46,080. That's
# perf-example work, not abstraction validation; not pursuing here.
# What IS validated end-to-end at BN=128: the non-pow2 N_STAGES=3
# phase cursor in `Pipeline{N}`.
#
# Configuration:
#   BM_CTA = 128, BN = 128, BK_TILE = 64, WGMMA_K = 16, N_STAGES = 3
...
#   Dynamic SMEM ~131 KB per CTA (opt-in via FUNC_ATTRIBUTE_MAX_DYNAMIC_SHARED_SIZE_BYTES)
#
...
function _ghh_gemm_kernel!(
    ...
    if wg_id == UInt32(0)
        ...
        while true
            ...
            # Unrolled epilogue stores. 16 groups (BN/8) × 2 packed stores
