...
# Register-file repartition (setmaxnreg is warpgroup-collective):
# 136*256 + 96*128 + 144*128 = 65536.
const FAB_NREG_SOFTMAX = 136
const FAB_NREG_CORR    = 96
const FAB_NREG_OTHER   = 144
...
@inline function _fab_load_kv(::Val{SLOT}, kv_ptr, tma, row::UInt32, mb,
                              ph::UInt32) where {SLOT}
    barrier_try_wait(mb + (FAB_BAR_KV_FREE + 8 * SLOT), ph)
...
@inline function _fab_pv_mma(::Val{STAGE}, ::Val{VIDX}, first_accum::Bool,
                             ::Val{PAR}, tmem::UInt32, dv0::UInt64,
                             idesc::UInt32, mb,
                             ph_pq::UInt32, ph_or::UInt32) where {STAGE, VIDX, PAR}
    d = tmem + UInt32(256 + STAGE * 128)
    for q in 0:3
        barrier_try_wait(mb + (FAB_BAR_P_Q + 32 * STAGE + 8 * q), ph_pq)
        if q == 0
            barrier_try_wait(mb + (FAB_BAR_O_RESC + 16 * STAGE + 8 * PAR), ph_or)
        end
...
@inline function _fab_softmax_body(first::Bool, oresc_par_ptr,
    ...
    if !first
        ...
        barrier_try_wait(sfree, ph_stats)
        ph_stats ⊻= UInt32(1)
        @inbounds stats[alpha_idx] = alpha
        ptx"bar.arrive"(stats_bar, UInt32(64))
    end

    # ── stream 4 chunks: ld(c+1) issues as soon as ld(c) lands, and flies
...
# in-flight PV.
@inline function _fab_corr_tile(::Val{STAGE}, mb, stats, alpha_idx::Int,
    ...
    barrier_try_wait(mb + (FAB_BAR_PV_DONE + 8 * STAGE), ph_pv)
    need = alpha < 1.0f0
...
function _fab_kernel!(
    ...
    # =====================================================================
    if tid >= UInt32(384)
        ptx"setmaxnreg.inc.sync.aligned.u32"(Val(FAB_NREG_OTHER))
    end
    ...
    if tid == FAB_LOAD_TID
        ...
        while lw < total_work
            ...
            barrier_try_wait(mb + FAB_BAR_Q_FREE, q_free_ph)
            q_free_ph ⊻= UInt32(1)
            barrier_arrive_expect_tx(mb + FAB_BAR_Q, FAB_TILE_BYTES)
            _fab_load_tile(q_ptr, 0, tma_Q, q_row0, mb + FAB_BAR_Q)
            krow, ph0 = _fab_load_kv(Val(0), kv_ptr, tma_K, krow, mb, ph0)
            barrier_arrive_expect_tx(mb + (FAB_BAR_Q + 8), FAB_TILE_BYTES)
            _fab_load_tile(q_ptr, FAB_TILE_BYTES, tma_Q, q_row0 + UInt32(FAB_BM),
                           mb + (FAB_BAR_Q + 8))
            vrow, ph1 = _fab_load_kv(Val(1), kv_ptr, tma_V, vrow, mb, ph1)
            krow, ph2 = _fab_load_kv(Val(2), kv_ptr, tma_K, krow, mb, ph2)
            vrow, ph3 = _fab_load_kv(Val(3), kv_ptr, tma_V, vrow, mb, ph3)
    ...
    if tid == FAB_MMA_TID
        ...
        while mw < total_work
            ...
            barrier_try_wait(mb + FAB_BAR_Q, ph_qa); ph_qa ⊻= UInt32(1)
            barrier_try_wait(mb + FAB_BAR_KV_FULL, kph0); kph0 ⊻= UInt32(1)
            _fab_qk_mma(Val(0), Val(0), tmem, dq0, dk0, idesc_qk, mb)
            barrier_try_wait(mb + (FAB_BAR_Q + 8), ph_qb); ph_qb ⊻= UInt32(1)
            ...
            barrier_try_wait(mb + (FAB_BAR_KV_FULL + 8), kph1); kph1 ⊻= UInt32(1)
            barrier_try_wait(mb + (FAB_BAR_KV_FULL + 16), kph2); kph2 ⊻= UInt32(1)
            ph_pq0, ph_or00 = _fab_pv_mma(Val(0), Val(0), true, Val(0),
                                          tmem, dv0, idesc_pv, mb, ph_pq0, ph_or00)
            _fab_qk_mma(Val(0), Val(1), tmem, dq0, dk0, idesc_qk, mb)
            ph_pq1, ph_or10 = _fab_pv_mma(Val(1), Val(0), true, Val(0),
            ...
            while trip < trips
                ...
                barrier_try_wait(mb + (FAB_BAR_KV_FULL + 24), kph3); kph3 ⊻= UInt32(1)
                barrier_try_wait(mb + FAB_BAR_KV_FULL, kph0); kph0 ⊻= UInt32(1)
                ph_pq0, ph_or01 = _fab_pv_mma(Val(0), Val(1), false, Val(1),
                                              tmem, dv0, idesc_pv, mb, ph_pq0, ph_or01)
                _fab_qk_mma(Val(0), Val(0), tmem, dq0, dk0, idesc_qk, mb)
                ph_pq1, ph_or11 = _fab_pv_mma(Val(1), Val(1), false, Val(1),
                ...
                barrier_try_wait(mb + (FAB_BAR_KV_FULL + 8), kph1); kph1 ⊻= UInt32(1)
                barrier_try_wait(mb + (FAB_BAR_KV_FULL + 16), kph2); kph2 ⊻= UInt32(1)
                ph_pq0, ph_or00 = _fab_pv_mma(Val(0), Val(0), false, Val(0),
                                              tmem, dv0, idesc_pv, mb, ph_pq0, ph_or00)
                _fab_qk_mma(Val(0), Val(1), tmem, dq0, dk0, idesc_qk, mb)
                ph_pq1, ph_or10 = _fab_pv_mma(Val(1), Val(0), false, Val(0),
                                              tmem, dv0, idesc_pv, mb, ph_pq1, ph_or10)
                _fab_qk_mma(Val(1), Val(1), tmem, dq0, dk0, idesc_qk, mb)
                _fab_commit(mb + (FAB_BAR_KV_FREE + 16))
                _fab_commit(mb + (FAB_BAR_KV_FREE + 8))
    ...
    # =====================================================================
    if tid < UInt32(256)
        ptx"setmaxnreg.inc.sync.aligned.u32"(Val(FAB_NREG_SOFTMAX))
        ...
        while sw < total_work
            ...
            barrier_try_wait(mb + FAB_BAR_EPI, epi_ph)
            epi_ph ⊻= UInt32(1)

            mneg = 0.0f0
            l_run = 0.0f0
            ph_s, ph_stats, mneg, pmax, sums, l_run = _fab_softmax_body(
                true, oresc, sp_base, sfull, sfree, pq_ptr, stats, alpha_idx,
                stats_bar, qk_scale, ph_s, ph_stats, mneg, pmax, sums, l_run)
            it = UInt32(1)
            while it < n_tiles
                po = (it & UInt32(1)) << UInt32(3)
                ph_s, ph_stats, mneg, pmax, sums, l_run = _fab_softmax_body(
                    false, oresc + Int(po), sp_base, sfull, sfree, pq_ptr,
    ...
    if UInt32(256) <= tid < UInt32(384)
        ptx"setmaxnreg.dec.sync.aligned.u32"(Val(FAB_NREG_CORR))
        ...
        while cw < total_work
            ...
            while it < n_tiles - UInt32(1)
                ...
                ph_pv0 = _fab_corr_tile(Val(0), mb, stats, alpha_idx0,
                                        stats_bar0, o_addr0, ponc, ph_pv0)
                ph_pv1 = _fab_corr_tile(Val(1), mb, stats, alpha_idx1,
            ...
            barrier_try_wait(mb + FAB_BAR_PV_DONE, ph_pv0); ph_pv0 ⊻= UInt32(1)
            barrier_try_wait(mb + (FAB_BAR_PV_DONE + 8), ph_pv1); ph_pv1 ⊻= UInt32(1)

            # ── epilogue: wait all PV done, normalize, store bf16 ──
            barrier_try_wait(mb + FAB_BAR_O_DONE, done_ph); done_ph ⊻= UInt32(1)
            ptx"tcgen05.fence::after_thread_sync"()
...
@testset "flash_attention_blackwell compiles at sm_100a" begin
    ...
    @test ptxas_compiles(_fab_kernel!, types; cap = v"10.0", feature_set = :arch,
                         minthreads = 512)
    ...
    @test ptxas_compiles(_fab_kernel!, types; cap = v"10.3", feature_set = :arch,
                         minthreads = 512)
    @test ptxas_compiles(_fab_kernel!, types; cap = v"10.0", feature_set = :family,
                         minthreads = 512)

    ptx = emit_ptx(_fab_kernel!, types; cap = v"10.0", feature_set = :arch,
                   minthreads = 512)
    ...
    # PV: TMEM A (bracketed register operand) — the new mma.tensor path
...
# ── Runtime — datacenter Blackwell only [10.0, 11.0) ────────────────────
