# Compile-touch shard: mma. The sweep engine, batching strategy, and the
...
# `op === :mma`. Together the shards must cover every wrapper opcode exactly once.

@testset "compile-touch (mma): every wrapper method compiles" begin
    result = compile_touch_sweep(op -> op === :mma)
    ...
    @test result.touched >= 150
    println("compile-touch mma: $(result.touched) wrapper methods compiled")
end
