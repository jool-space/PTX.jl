# Compile-touch shard: tcgen05. The sweep engine, batching strategy, and the
...
# `op === :tcgen05`. Together the shards must cover every wrapper opcode exactly once.

@testset "compile-touch (tcgen05): every wrapper method compiles" begin
    result = compile_touch_sweep(op -> op === :tcgen05)
    ...
    @test result.touched >= 220
    println("compile-touch tcgen05: $(result.touched) wrapper methods compiled")
end
