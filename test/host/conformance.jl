...
const PROBES = Tuple{String, Tuple, String, String, Regex}[
    ...
    ("llvm.nvvm.barrier.cta.red.popc.aligned.all",
        (UInt32, Bool), "sm_70", "+ptx60",
        r"bar\.red\.popc\.u32 \s*%r\d+, %r\d+, %p\d+;"),
    ("llvm.nvvm.barrier.cta.red.popc.aligned.count",
        (UInt32, UInt32, Bool), "sm_70", "+ptx60",
        r"bar\.red\.popc\.u32 \s*%r\d+, %r\d+, %r\d+, %p\d+;"),
    ("llvm.nvvm.barrier.cta.red.popc.all",
        (UInt32, Bool), "sm_70", "+ptx60",
        r"barrier\.red\.popc\.u32 \s*%r\d+, %r\d+, %p\d+;"),
    ("llvm.nvvm.barrier.cta.red.popc.count",
        (UInt32, UInt32, Bool), "sm_70", "+ptx60",
        r"barrier\.red\.popc\.u32 \s*%r\d+, %r\d+, %r\d+, %p\d+;"),
    ("llvm.nvvm.barrier.cta.red.and.aligned.all",
        (UInt32, Bool), "sm_70", "+ptx60",
        r"bar\.red\.and\.pred \s*%p\d+, %r\d+, %p\d+;"),
    ("llvm.nvvm.barrier.cta.red.and.aligned.count",
        (UInt32, UInt32, Bool), "sm_70", "+ptx60",
        r"bar\.red\.and\.pred \s*%p\d+, %r\d+, %r\d+, %p\d+;"),
    ("llvm.nvvm.barrier.cta.red.and.all",
        (UInt32, Bool), "sm_70", "+ptx60",
        r"barrier\.red\.and\.pred \s*%p\d+, %r\d+, %p\d+;"),
    ("llvm.nvvm.barrier.cta.red.and.count",
        (UInt32, UInt32, Bool), "sm_70", "+ptx60",
        r"barrier\.red\.and\.pred \s*%p\d+, %r\d+, %r\d+, %p\d+;"),
    ("llvm.nvvm.barrier.cta.red.or.aligned.all",
        (UInt32, Bool), "sm_70", "+ptx60",
        r"bar\.red\.or\.pred \s*%p\d+, %r\d+, %p\d+;"),
    ("llvm.nvvm.barrier.cta.red.or.aligned.count",
        (UInt32, UInt32, Bool), "sm_70", "+ptx60",
        r"bar\.red\.or\.pred \s*%p\d+, %r\d+, %r\d+, %p\d+;"),
    ("llvm.nvvm.barrier.cta.red.or.all",
        (UInt32, Bool), "sm_70", "+ptx60",
        r"barrier\.red\.or\.pred \s*%p\d+, %r\d+, %p\d+;"),
    ("llvm.nvvm.barrier.cta.red.or.count",
        (UInt32, UInt32, Bool), "sm_70", "+ptx60",
        r"barrier\.red\.or\.pred \s*%p\d+, %r\d+, %r\d+, %p\d+;"),
...
# completeness against the wrapper registry's :mma / :mma_scaled name sets is
...
function _mma_sp_sweep!(shape, a, b, c; ordered = false)
    ...
    NVVM.isintrinsic(name) || return   # would land as a :missing registry record
...
@testset "tcgen05 ws mma generated family: full probe coverage" begin
    names = PTX.wrapper_intrinsic_names(:tcgen05_mma_ws)
    @test length(names) == 8
...
@testset "tcgen05 sparse mma generated family: full probe coverage" begin
    names = PTX.wrapper_intrinsic_names(:tcgen05_mma_sp)
...
@testset "tcgen05 dense mma generated family: full probe coverage" begin
    names = PTX.wrapper_intrinsic_names(:tcgen05_mma_dense)
    @test length(names) == 18
    registry = [n for n in keys(PTX.NVVM.TABLE)
...
@testset "mma generated families: full probe coverage" begin
    ...
    @test length(PTX.wrapper_intrinsic_names(:mma_sp)) == 44
    @test length(PTX.wrapper_intrinsic_names(:mma_sp_ordered)) == 44
    # the integer sparse subset (records whose mods carry :s32)
    ...
    @test _MMA_SP_ORDERED_SWEPT ==
          Set(PTX.wrapper_intrinsic_names(:mma_sp_ordered))
    @test _MMA_SCALED_SWEPT == Set(PTX.wrapper_intrinsic_names(:mma_scaled))
