...
# name => (argtypes, mcpu, mattr, expected instruction regex)
const PROBES = Dict(
...
@testset "every wrapper intrinsic has a selection probe" begin
    ...
    @test !isempty(used)
    unprobed = sort!(collect(setdiff(used, keys(PROBES))))
    isempty(unprobed) ||
...
@testset "selection probes through the artifact llc" begin
    exe = llc().exec[1]
    for (name, (argtypes, mcpu, mattr, expect)) in sort!(collect(PROBES); by=first)
