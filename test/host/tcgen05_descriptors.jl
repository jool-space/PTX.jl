...
# ── PTX 9.4 §9.7.18.4: the architecture-gated descriptor encodings ──────────
# The 15-bit shared-descriptor fields and the lut::b segment bit (Table 49),
# the wide-K and scale-layout bits (Tables 51–53), and the `.kind::ti16`
# instruction descriptor. Same double-entry rule as above; every default
# keeps the pre-9.4 encoding, which the sweeps above already pin.

using PTX: tcgen05_instr_desc_ti16
...
@testset "tcgen05 shared descriptor: 15-bit fields and lut segment bit" begin
    ...
    @test _td_field(top, 0, 15) == 0x7fff
    @test _td_field(top, 15, 1) == 0
    @test _td_field(top, 16, 15) == 0x7fff
    @test _td_field(top, 31, 1) == 0
    @test _td_field(top, 32, 14) == 0x3fff
    ...
    for dt in (:e2m3, :e3m2, :e2m1)
        ...
        @test tcgen05_instr_desc_f8f6f4(; args..., a_dtype = dt,
              ...
              tcgen05_instr_desc_f8f6f4(; args..., a_dtype = dt,
                                        a_major = :MN, k = 32)
        @test _td_field(tcgen05_instr_desc_f8f6f4(; args..., a_dtype = dt,
                                                   k = 64), 29, 1) == 1
    end
    @test _td_field(tcgen05_instr_desc_f8f6f4(; args..., a_major = :MN,
                                               b_major = :MN, k = 64),
                     29, 1) == 1
...
@testset "tcgen05 mxf8f6f4 idesc: wide K, scale layout, lut::b rules" begin
    ...
    for dt in (:e2m3, :e3m2, :e2m1)
        ...
        @test _td_field(tcgen05_instr_desc_mxf8f6f4(
...
@testset "tcgen05 mxf4/mxf4nvf4 idesc: K code and scale layout" begin
    ...
    both = tcgen05_instr_desc_mxf4nvf4(; nvf4..., k = 128, scale_a_layout = 128,
                                       sparsity_version = 1)
    @test _td_field(both, 3, 1) == 1
    @test _td_field(both, 12, 1) == 1
    @test _td_field(both, 23, 2) == 2
    @test _td_field(both, 26, 1) == 1
    @test _td_field(both, 31, 1) == 0
end
...
const _TD_TI16_ZERO_MASK = UInt32(0x3) | (UInt32(0x1) << 3) |
    (UInt32(0x1) << 6) | (UInt32(0x3) << 15) | (UInt32(0x1) << 23) |
    (UInt32(0x1) << 29)

_td_expected_ti16(m, n, sa, sb, sparse, shift) =
...
@testset "tcgen05 ti16 idesc: exhaustive public fields" begin
    ...
    for m in (32, 64, 128, 256), n in 8:8:256, sa in (1, -1), sb in (1, -1),
        ...
        checked += 1
        desc == _td_expected_ti16(m, n, sa, sb, sparse, shift) &&
            iszero(desc & _TD_TI16_ZERO_MASK) &&
            _td_field(desc, 4, 2) == 2 &&
            _td_field(desc, 7, 3) == 3 && _td_field(desc, 10, 3) == 3 ||
            (mismatches += 1)
    end
    @test mismatches == 0
