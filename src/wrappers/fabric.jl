...
# try_wait variants).
#
# Hand-written: the two-register CFT handle operand `[leId, off]` (u32 +
# u64 inside one bracketed group) has no chain-default rendering, and several
# pointer-constraint shapes occur per call (shared `r`, fabric handle scalars
# `r`/`l`).
#
# Scope: this file ships the operand-shape variants the rest of PTX.jl's
# wrappers cover at minimum — `submit`, `wait`, `try_get`, `try_put` (basic
# + .multimem). DEFERRED:
...
# pinned at registration. Sole exception: callers can choose to also branch
# on a different completion mechanism in the future (none today).
