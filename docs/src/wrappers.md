...
| Family | File | Surface |
|---|---|---|
| `bar` / `barrier` (CTA) | `wrappers/barrier.jl` | `bar.{sync,arrive}`, `bar.warp.sync`, `barrier.{sync,arrive}[.aligned]`, and the reductions `bar.red.{popc.u32,and.pred,or.pred}` / `barrier.red.{popc,and,or}[.aligned].{u32,pred}` lower to `llvm.nvvm.barrier.cta.*` for `Val`, `UInt32`, and `Int32` barrier ids and thread counts. A reduction takes its predicate as a trailing `Bool` (spell `{!}c` as `!c`); `.popc` returns `UInt32`, `.and`/`.or` return `Bool`. `bar.*` and `barrier.*.aligned` emit the same `bar.*` instruction. Wider integer operands fall back to the chain's inline asm. |
...
| `cvt` sub-byte FP packing | `wrappers/cvt.jl` | `cvt.rn.satfinite.e2m1x2.{f32,f16x2,bf16x2}` pack and `cvt.rn.{f16x2,bf16x2}.e2m1x2` unpack — `.b8` carrier through a `mov.b16` brace-pair shim because NVPTX inline asm has no `.b8` register class |
...
| `tcgen05_instr_desc_ti16` | `.kind::ti16` instruction descriptor (s1z4m11 → s32, PTX ISA 9.4, sm_107f family). Negation is supported; transpose is not caller-controlled because the ISA's descriptor table and its transpose-support table disagree. |
| `tcgen05_instr_desc_f8f6f4` | `.kind::f8f6f4` instruction descriptor (independent e4m3/e5m2/e2m3/e3m2/e2m1 A/B types, f16 or f32 destination). `k = 64` selects the sm_107f-family wide-K encoding; `lut_b = true` checks the `.decompress::lut::b` operand rules. |
| `tcgen05_instr_desc_mxf8f6f4` | `.kind::mxf8f6f4` block-scale instruction descriptor (scale-factor data IDs; scale matrix type is the ISA-fixed UE8M0; sm_107f-family `k` and `scale_a_layout` encodings; `lut_b` operand rules). |
| `tcgen05_instr_desc_mxf4`, `tcgen05_instr_desc_mxf4nvf4` | `.kind::mxf4` / `.kind::mxf4nvf4` block-scale instruction descriptors (E2M1 elements; transpose ISA-unsupported; nvf4 additionally chooses the scale matrix type; sm_107f-family `k` and `scale_a_layout` encodings). |
...
negate keywords and the mxf4 and ti16 builders no transpose keywords
(ISA-unsupported, or ISA-contradictory, for those kinds). The
architecture-gated encodings (the `sm_107f`-family K-dimension and
scale-layout bits, the 15-bit shared-descriptor fields, the
`.decompress::lut::b` segment bit) default to zero and are set only through
their explicit keywords (`k`, `scale_a_layout`, `wide`, `lut_k_offset`).
An illegal descriptor is unrepresentable through the builders, not merely
rejected. Matrix addresses and byte offsets must be 16-byte aligned and fit
the 18-bit descriptor input window (19-bit with `wide=true`); invalid
