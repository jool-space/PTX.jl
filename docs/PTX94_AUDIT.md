...
| `tcgen05.ld{.red}.spcompress`, register `spcompress/spdecompress` | Bound: 72 `ld{.red}.spcompress` forms, 28 `spcompress` and 143 `spdecompress` forms (the five ISA sizing conditions enumerated independently in the host oracle), `spcompress_desc` for Table 38; all assemble at sm_107a and are rejected at sm_107f/sm_100a. No numeric evidence (sm_107a hardware). |
| `applypriority.async.bulk{.tensor}` | Bound: the bulk form through the chain under a sink contract, the tensor form as 18 typed wrappers (tile 1d–5d, base im2col 3d–5d, tile overrides); assembled at sm_107f/sm_107a. |
| TMA report mechanism, overrides, im2col-no-offset W, eviction priority | Bound as a closed inventory of 360 typed wrappers over `cp.async.bulk.tensor` loads/stores, `cp.async.bulk.prefetch.tensor`, and `cp.reduce.async.bulk.tensor` (including the previously unwrapped base tile/`im2col_no_offs` forms), with the cuts recorded in `src/wrappers/tma.jl`; all assemble at sm_107f/sm_107a and are rejected at sm_103f/sm_120f/sm_107. No runtime evidence. |
